...
class SymbolicRegressor:
    ...
    def tune_new_term(self, X, g_order, theta_0):
        ...
        # CNN fix
    def CNNtune_new_term(self, X, g_order, theta_0):
        # Tunes a new term for the model for f with a Meijer G-function of order g_order
        shape = X[0].shape
        X = np.array([x*1./np.max(np.linalg.eig(X[i])) for x in X])
        ...
        def split_theta(theta):
            ...
            theta_g = np.concatenate((theta[:p + q], np.array([1.0])))
            theta_w = theta[-1]
            theta_v = theta[p + q: p + q + shape[0]]
            theta_u = theta[p + q + shape[0]:p + q + shape[0] + shape[1]]
            return theta_g, theta_v, theta_u, theta_w

        def loss(theta):
            # Computes the loss for a new term of parameter theta
            residual_list = self.current_resi
            theta_g, v_, u_, w_ = split_theta(theta)
            meijer_g_ = MeijerG(theta=theta_g, order=g_order)

            v_ = v_.reshape(-1, 1)
            u_ = u_.reshape(-1, 1)

            print (X.shape, v_.shape, u_.shape)
            left_imp = np.matmul(X, v_)
            Projection  = np.matmul(left_imp, u_)
            Normalization = np.linalg.norm(v_)*np.linalg.norm(u_)
            Y = w_ * meijer_g_.evaluate(ReLU(Projection*1./Normalization))
        ...
        new_theta, new_loss = self.optimize_CG(loss, theta_0)
        new_theta_meijer, new_v, new_u, new_w = split_theta(new_theta)
        new_meijerg = MeijerG(theta=new_theta_meijer, order=g_order)
        return new_meijerg, new_v, new_u, new_w, new_loss
    ...
    def CNNfit(self, f, X):
        ...
        while current_loss > loss_tol:
            ...
            new_terms_list = []
            np.random.seed(self.random_seed)
            v0 = np.random.randn(shape[0])
            u0 = np.random.randn(shape[1])
            self.current_resi = Y_target - self.predict(X)
            if self.verbosity:
                print(100 * "%")
                print("Now working on term number ", count, ".")
            for k in range(len(h_dic)):
                ...
                new_meijer_g, new_v, new_u, new_w, new_loss = self.CNNtune_new_term(X, g_order, theta_0)
                new_loss_list.append(new_loss)
    ...
    def CNNbackfit(self, f, X):
        ...
        for k in range(len(self.terms_list) - 1):
            if self.verbosity:
                ...
                print("Now backfitting term number ", k + 1, ".")
            self.current_resi = f(X) - self.predict(X, exclude_term=True, exclusion_id=k)
            meijer_g0, v0, u0, w0 = self.terms_list[k]
            theta_meijer0 = meijer_g0.theta[:-1]
            theta0 = np.concatenate((theta_meijer0, v0, u0, [w0]))
            g_order = meijer_g0.order
            new_meijerg, new_v, new_u, new_w, new_loss = self.CNNtune_new_term(X, g_order, theta0)
            if new_loss < self.loss_list[-1]:
                self.terms_list[k] = [new_meijerg, new_v, new_u, new_w]
                self.loss_list[-1] = new_loss
        if self.verbosity:
            print(100 * "=")
            print("Backfitting complete.")